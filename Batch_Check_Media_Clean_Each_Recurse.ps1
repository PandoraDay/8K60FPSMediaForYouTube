...
# ---- Condition 03: Timebase Check ----
        ...
        if ($fpsOutput -match "time_base=1/60000") {
            if ($fpsOutput -match "time_base=1/48000") {
                Write-Host "Time_base matches 1/60000 and 1/48000." -ForegroundColor Green
            } elseif ($fpsOutput -match "time_base=1/44100") {
                Write-Host "Time_base matches 1/60000 and 1/44100." -ForegroundColor Green
            } else {
                Write-Host "Secondary time_base mismatch." -ForegroundColor Red
                $failures += "Secondary time_base mismatch"
            }
        } else {
            Write-Host "Time_base=1/60000 not found." -ForegroundColor Red
            $failures += "Time_base=1/60000 not found"
        }
